...
public class UserControllerTest extends CommonControllerTest {
	...
	void shouldReturn201CreatedOnSignup() throws Exception {
		// Given
	...
	void shouldReturn200OkOnLogin() throws Exception {
		// Given
	...
	void shouldReturn200OkOnLogout() throws Exception {
		// When & Then
	...
	void shouldCheckNickname() throws Exception {
		// Given
	...
	void shouldCheckEmail() throws Exception {
		// Given
	...
	void shouldReturn200OkIfAuthenticatedOnWithdraw() throws Exception {
		// Given
		Users mockUser = Users.builder()
			.email("test@example.com")
			.password("password123")
		...
		// When & Then
		mockMvc.perform(post("/api/user/withdraw")).andExpect(status().isOk());

		Mockito.verify(userService, Mockito.times(1)).deleteUser(eq(mockUser.getEmail()));
		Mockito.verify(sessionConfig, Mockito.times(1)).invalidateSession(any(HttpServletRequest.class));
	}

	@Test
	@DisplayName("POST /withdraw 요청 시 인증되지 않은 사용자라면 401 UNAUTHORIZED 상태를 반환해야 한다")
	void shouldReturn401IfNotAuthenticatedOnWithdraw() throws Exception {
		// Given
		Mockito.when(sessionConfig.getSessionUser(any(HttpServletRequest.class))).thenReturn(null);

		// When & Then
		mockMvc.perform(post("/api/user/withdraw")).andExpect(status().isUnauthorized());

		Mockito.verify(userService, Mockito.times(0)).deleteUser(any(String.class));
	}

	@Test
	@DisplayName("유효하지 않은 이메일로 회원가입 시 400 Bad Request")
	void shouldReturn400ForInvalidEmailOnSignup() throws Exception {
		// Given
	...
	void shouldReturn400ForInvalidEmailOnLogin() throws Exception {
		// Given
	...
	void shouldReturn400ForInvalidEmailOnCheckEmail() throws Exception {
		// Given
	...
	void shouldReturn400ForEmptyNicknameOnCheckNickname() throws Exception {
		// Given
